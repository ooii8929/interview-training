...
export default function Video(props) {
  ...
  const [runCodeResponseOutput, setRunCodeResponseOutput] =
    React.useState(null);
  const [runCodeResponseExpect, setRunCodeResponseExpect] =
    React.useState(null);
  const [changeStatus, setChangeStatus] = React.useState("");

  const [runCodeResponseStatus, setRunCodeResponseStatus] =
    React.useState(null);
  ...
  async function getUserCodeLog(questionID) {
    let codeLog = await axios.get(
      "http://localhost:3001/api/1.0/user/code/log",
      {
        params: {
  ...
  async function submitAnswer(e) {
    response = await axios.post(
      "http://localhost:3001/api/1.0/training/submit/compile/",
      {
        user_id: userId,
  ...
  async function runCode() {
    try {
      response = await axios.post(
        "http://localhost:3001/api/1.0/training/run/compile",
        {
          question_id: questionID,
          language: language,
          content: code,
  ...
  function setNowQuestion(profileQuestion) {
    if (profileQuestion) {
      ...
      // 4. 如果有，設定題號
      if (notFinishedQuestion)
        setNowQuestionNumber(notFinishedQuestion[0]["qid"]);
    }
  }
  // 5.設定當前題號
  React.useEffect(
    (e) => {
      if (nowQuestionNumber)
        console.log("5. 設定當前題號成功：", nowQuestionNumber);
      // 5. 用當前題號拿到題目
  ...
  async function shareAnswer(n) {
    try {
      response = await axios.post(
        "http://localhost:3001/api/1.0/article/code",
        {
          user_id: userId,
          question_id: questionID,
          code: code,
        ...
        <div className="code_container">
          ...
          >
            <Grid item xs={6} className="code-description-container">
              <div
                dangerouslySetInnerHTML={{ __html: question[0].description }}
              ></div>
            </Grid>
            <Grid item xs={6}>
