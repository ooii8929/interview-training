...
export default function SocialArticle() {
  ...
  const baseURL = "http://localhost:3001/api/1.0/article/id";
  ...
  async function postGood(e) {
    ...
    try {
      let res = await axios.post(
        "http://localhost:3001/api/1.0/article/good",
        postDetail
      );
  ...
  async function postBad(e) {
    ...
    try {
      let res = await axios.post(
        "http://localhost:3001/api/1.0/article/bad",
        postDetail
      );
  ...
  useEffect(
    (e) => {
      async function getArticles() {
        try {
          let tmpAllArticles = await axios.get(
            "http://localhost:3001/api/1.0/article",
            {
              params: {
                profession: language,
              },
          ...
          <div>
            <Grid container spacing={4}>
              ...
              <Grid item xs={2} className="article-main-left">
                ...
                />
                <div
                  className="tringle good"
                  onClick={postGood}
                  ref={tringleGood}
                ></div>
                {articles ? <div>{goods}</div> : null}
                <div
                  className="tringle-bad good"
                  onClick={postBad}
                  ref={tringleBad}
                ></div>
              </Grid>
